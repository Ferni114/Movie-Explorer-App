    ...
    final databaseOptions = sqflite.OpenDatabaseOptions(
      ...
      onCreate: (database, version) async {
        await database.execute(
            'CREATE TABLE IF NOT EXISTS `favorites` (`id` INTEGER NOT NULL, `poster` TEXT NOT NULL, `title` TEXT NOT NULL, `date` TEXT NOT NULL, `ranking` TEXT NOT NULL, PRIMARY KEY (`id`))');
...
class _$FavoritesDAO extends FavoritesDAO {
  ...
  @override
  Future<List<FavoriteModel>> getFavorites(int page) async {
    return _queryAdapter.queryList('select * from favorites limit ?1',
        mapper: (Map<String, Object?> row) => FavoriteModel(
            id: row['id'] as int,
            poster: row['poster'] as String,
            title: row['title'] as String,
            date: row['date'] as String,
            ranking: row['ranking'] as String),
        arguments: [page]);
  }
